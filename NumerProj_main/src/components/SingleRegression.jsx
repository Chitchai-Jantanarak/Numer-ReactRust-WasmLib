...
function SingleRegression() {
  ...
  function resultDegreeSetter(result) {
  ...
  const calculateRegression = async () => {
    try {
      ...
      // Set in form of degree x
      regressionResult.answer = resultDegreeSetter(regressionResult.answer);
  ...
  const plotData = result ? (() => {
    ...
    // May collapse by many number (O(n))
    const xFuncValues = Array.from( { length: (xMaxBound - xMinBound) * 1 }, (_, i) => xMinBound + (i * 1) );
